...
ipcMain.handle('units:add', (event, { product_id, codigo, costoUnitario, usuario }) => {
  ...
  const transaccion = db.transaction(() => {
    ...
    ).run(product_id, product.tipo, product.nombre, costo, costo, usuario || '', unitInfo.lastInsertRowid);
  });
  transaccion();
  return { ok: true };
});
...
ipcMain.handle('units:addRange', (event, { product_id, codigoInicio, codigoFin, costoUnitario, usuario }) => {
  ...
  // Un registro de cargo (compras) por cada codigo individual del rango, en vez de un unico
  // renglon agregado con la cantidad total. Asi cada IMEI/codigo queda visible y con su propia
  // secuencia (numero de fila) en el registro de cargos/descargos, igual que ya ocurre con los
  // descargos por unidad.
  ...
  const transaccion = db.transaction((codigos) => {
    for (const codigo of codigos) {
      ...
      insertCompra.run(product_id, product.tipo, product.nombre, costo, costo, usuario || '', unitInfo.lastInsertRowid);
      agregados++;
    }
  });
