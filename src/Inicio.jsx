...
// Widget de metas: solo muestra el % de avance hacia la meta mensual (sin montos de dinero, en
// el mismo espiritu del resto del dashboard de Inicio). El vendedor ve solo su propia barra; el
// administrador ve la de todo el equipo. Si nadie tiene meta configurada, el widget no se
// muestra (no tiene sentido una barra de progreso sin meta).
function MetaProgresoWidget({ user }) {
  const [progreso, setProgreso] = useState(null);

  useEffect(() => { window.api.getProgresoMetas().then(setProgreso); }, []);

  if (!progreso) return null;

  const vendedores = user.role === 'administrador'
    ? progreso.vendedores
    : progreso.vendedores.filter((v) => v.usuario === user.username);
  const conMeta = vendedores.filter((v) => v.metaMensualUsd > 0);
  if (conMeta.length === 0) return null;

  return (
    <div style={{ ...cardStyle, marginBottom: '10px' }}>
      <h3 style={{ margin: 0, fontSize: '0.9rem' }}>🎯 Meta del mes ({progreso.mes})</h3>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '6px' }}>
        {conMeta.map((v) => (
          <div key={v.usuario}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', color: '#475467', marginBottom: '2px' }}>
              <span>{user.role === 'administrador' ? v.nombreVendedor : 'Tu avance'}</span>
              <span style={{ fontWeight: 600 }}>{Math.round(v.progresoPct)}%</span>
            </div>
            <div style={{ background: '#eaecf0', borderRadius: '6px', height: '8px', overflow: 'hidden' }}>
              <div style={{ width: `${v.progresoPct}%`, height: '100%', background: v.progresoPct >= 100 ? VERDE : AZUL }} />
            </div>
          </div>
        ))}
      </div>
